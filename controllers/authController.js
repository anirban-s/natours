...
const createSendToken = (user, statusCode, res) => {
  ...
  // to hide password while creating new user
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  try {
    await sendEmail({
      email: user.email,
      subject: 'Yoour password reset token (valid for 10 min)',
      message
    });
...
exports.resetPassword = catchAsync(async (req, res, next) => {
  ...
  // 3) Update changedPasswordAt property for the user in schema
...
exports.updatePassword = catchAsync(async (req, res, next) => {
  // 1) Get user from the collectuon
